...
(deftype ThreadedOutputSplitter [source sinks]
  proto/Splitter
  (connect
    [this sink operation-map]
    ;; Simply puts the sink and it's operation-map into sinks vector
  ...
  (commit
    [this]
    (doseq [[sink op-map] @sinks]
      (connect-threaded source sink op-map))
    this))
...
(defn output-splitter
  ...
  ([source execution-mode]
   (cond
     ...
     (= execution-mode :multi-thread) (ThreadedOutputSplitter. source (atom []))
...
(comment
  (let [a (streams/stream 10)
        b (streams/stream 10)
        c (streams/stream 10)
        splitter (output-splitter a :multi-thread)
        read (fn [x y]
               (streams/consume
                (fn [v]
                  (Thread/sleep 100)
