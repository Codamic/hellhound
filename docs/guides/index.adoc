= HellHound Documentation
:Revision:  1.0.0-alpha1
:Author:    Codamic Technologies
:Email:     <devs@codamic.tech>
:homepage: http://hellhoud.io
:icons: font
:iconsfont:   font-awesome
:sectanchors: true
:description: A fullstack, async framework for creating web applications using Clojure and Clojurescript
:toc: macro
:toc: left
:toclevels: 3
:toc-title: HellHound Documentation
:doctype: book
:sectnumlevels: 3
:keywords: Clojure, Clojurescript, clj, cljs, framework, stream, web, aleph, stream processing, async, fullstack
:docinfo: shared
:type: reference
:source-highlighter: coderay
:imagesdir: ./images/
:front-cover-image: image:cover.svg[Front Cover,1050,1600]

[IMPORTANT]
.Under development software
====
This software is not ready to use yet. It's under heavy development
and reaches the alpha stage pretty soon. If you like to be part of this process please
leave us a message.
====

ifdef::backend-html5[]
image::https://github.com/Codamic/hellhound/raw/master/assets/hellhound-white.png[HellHound,align="center"]
++++
<div class="center-aligned download-btns">
<a href="/guides/hellhound.pdf" class="lxbtn btn-success">
   <i class="fa fa-download"></i>
   PDF
</a>

<a href="/guides/hellhound.epub" class="lxbtn btn-success">
   <i class="fa fa-download"></i>
   EPUB
</a>

<a href="/guides/hellhound.mobi" class="lxbtn btn-success">
   <i class="fa fa-download"></i>
   MOBI
</a>
</div>
++++
endif::[]

---
:sectnums:

== What is HellHound?
*HellHound* is a set of libraries to create simple and elegant programs based on streams. An *HellHound* application
basically is a <<_systems,system>> of <<_components,components>> which work together in order to achieve a common goal.
Components form one or more data pipelines through <<_workflow_2,workflows>>. In general systems are a great way to
manage the lifecycle and data flow or your program and components are awesome for managing the state and dependencies
of different pieces of your program.

HellHound provides different built-in components for different types of systems. For example Webserver component for creating
a fullstack web application, or a kafka component for a data processing application.

=== Goals
The ultimate goal behind *HellHound* is to create a set of libraries for creating well orginized, simple, easy to test, easy to
to scale and distribute applications using Clojure and Clojurescipt around streams and data pipelines.

Another goal of this project is to create a tool which allows developers to focus on their business logic as much as possible
while *HellHound* takes care of different aspects of their distributed and scalable software in an opinionated way. We want
to achieve this using different component which provides these kind of tasks.


// Prerequisite
include::prerequisite/index.adoc[leveloffset=+1]

// Getting Started
include::getting_started/index.adoc[leveloffset=+1]

// System
include::systems/index.adoc[leveloffset=+1]

// Components
include::components/index.adoc[leveloffset=+1]

// HTTP
include::http/index.adoc[leveloffset=+1]

// Appendix 1
include::appendix/contribute.adoc[leveloffset=+1]

// Appendix 2
include::appendix/code_of_conduct.adoc[leveloffset=+1]


[colophon]
= Colophon

The HellHound User Manual

(C) 2017 by Sameer Rahmani <lxsameer@gnu.org>

Created in Asciidoctor, Debian Unstable and FG42 Editor.
